...
class CadreRepository with ChangeNotifier{
  ...
  //5. Bỏ phiếu
  Future<bool> CadreVote(BuildContext context,ElectionUserHavePaticipanted_Model electionDetail, String ID_object, BigInt GiaTriPhieu) async{
    try{
      final accessToken = await _tokenRepository.getAccessToken();
      String uri = cadreVote;
      print("path: $uri");
  ...
  Future<List<VotingHistoryModel>> GetListOfVotingHistory(BuildContext context, String ID_CanBo) async{
    try{
      final accessToken = await _tokenRepository.getAccessToken();
      String uri = getListOfVotingHistory_Cadre+ID_CanBo;
      print('Path: ${uri}');
